...
Gui.update_lead_content = function()
  local list = {}
  local caption = ''
  local not_leader_caption = ''
  local place = 0
  local leaders = {}
  local playername = nil
  local color = nil
  for name, data in pairs(DB.players_data) do
    list[#list + 1] = {name = name, kills = data.ffa_kills, deaths = data.ffa_deaths}
  end
  ...
  for _, player in pairs(game.connected_players) do
    ...
    if player.surface.index == 1 then
      if leaders[playername] then --자신이 3위 내에 있는 경우
        not_leader_caption = ''
      else --순위 밖
        place = 0
        for i = 1, #list do
        ...
        color = player.color
        color = '[color='..tostring(color.r)..','..tostring(color.g)..','..tostring(color.b)..']'
        not_leader_caption = '   '..tostring(place)..'. '..color
          ..playername..'[/color] ('
          ..PDB.ffa_kills..'/'
          ..PDB.ffa_deaths..')'
      end
      PDB.guis.ffa_frame.lead_content.caption = '[font=count-font]'..caption..not_leader_caption..'[/font]'
